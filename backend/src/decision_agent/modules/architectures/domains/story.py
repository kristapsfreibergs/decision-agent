...
WORKER_CATALOG: list[dict[str, Any]] = [
    {
        ...
        "parallelizable": True,
        "role": "briefer",
        "goal_template": "Extract story intent from the task. Define: genre, target audience, approximate length, key characters, setting, mood, and any specific elements the human wants included. Fill gaps with sensible defaults — do not block unless the task is completely empty. Check archive/knowledge/story/ for past preferences.",
        "read_paths": ["archive/knowledge/story/**"],
    ...
    },
    {
        ...
        "parallelizable": True,
        "role": "researcher",
        "goal_template": "Research the current market for this story type. Find: what is selling well in the target market right now, genre conventions and reader expectations, cultural traditions and sensitivities for the target audience. Use web search if available, fall back to archive/knowledge/markets/ if not.",
        "read_paths": ["archive/knowledge/markets/**"],
    ...
    },
    {
        ...
        "parallelizable": False,
        "role": "stylist",
        "goal_template": "Apply the human's personal writing voice and style to the proofread story. Read their past writing samples from archive/knowledge/voice/ to understand their style patterns (sentence length, vocabulary, rhythm, tone). If no samples exist, ask what style they prefer before proceeding. Produce the final polished story.",
        "read_paths": [
            "data/runs/{run_id}/workspace/proofread.md",
            "data/runs/{run_id}/workspace/brief.md",
            "archive/knowledge/voice/**",
        ],
